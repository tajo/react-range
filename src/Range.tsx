...
class Range extends React.Component<IProps> {
  ...
  onMouseDownTrack = (e: React.MouseEvent) => {
    ...
    if (values.length > 1 && draggableTrack) {
      ...
      const draggedThumbIndex = getClosestThumbIndex(this.thumbRefs.map(t => t.current!), e.clientX, e.clientY, direction)
  ...
  onTouchStartTrack = (e: React.TouchEvent) => {
    ...
    const { values, draggableTrack } = this.props;
